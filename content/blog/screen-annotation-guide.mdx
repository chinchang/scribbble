...
Worth knowing: several live-annotation tools now ship a single-user whiteboard mode ([Scribbble](/) and ZoomIt both do), which drops a blank canvas over your screen on a hotkey. That covers the mid-presentation sketch without the context switch into Miro. It doesn't replace a real collaborative whiteboard (nobody else can draw on it), but the mid-presentation sketch is the case most people actually reach for.
...
Rectangles frame a region. The header. This entire panel. Everything above the fold. Ellipses do the same job for round things: an avatar, a status dot, the cursor sitting on a button. Use either with a thick outline and no fill so the underlying interface stays visible. A box and an arrow do different things; boxes group, arrows point.
...
Zoom isn't really an annotation, but it's a close cousin (it's literally ZoomIt's namesake, and [Scribbble](/) ships it as Magnify). Magnifies a portion of the screen. Use it when even an arrow won't help; skip it when you could just sit closer to the camera or share a closer crop instead.
